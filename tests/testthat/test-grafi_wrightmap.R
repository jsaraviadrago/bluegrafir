test_that("Wright map", {
  ...
  items.data <- cbind(items.data,
                      items_c = ifelse(items.data$items_n < 10,
                                       paste("IT_EX",
                                             items.data$items_n,
                                             sep="_0"),
                                       paste("IT_EX",
  ...
  items.measure <- data.frame(items.data$items)
  ...
  items.data <- data.frame(
    ...
    items.measure,
    items_c = items.data$items_c,
    stringsAsFactors=F)
  groups <- 1:groups
  groups <- paste0("D",groups)

  pe <-
    ggplot2::ggplot(personas.data, ggplot2::aes(x=personas)) +
    ggplot2::coord_flip() +
  ...
  it <-
    ggplot2::ggplot(items.data, ggplot2::aes(x=items.measure, y=items_g)) +
    ggplot2::coord_flip() +
    ggplot2::geom_jitter(position = ggplot2::position_jitter(height = .1), size=2,
                         ggplot2::aes(color = "#339cff")) +
    geom_text_repel(ggplot2::aes(items.measure, items_g, label = items_c), size = 2) +
